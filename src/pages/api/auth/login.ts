...
export const POST: APIRoute = async ({ request, cookies, url }) => {
  ...
  if (provider === "google") {
    ...
    if (session) return new Response(JSON.stringify({ redirect: "/admin/dashboard" }), { status: 200 });
  ...
  if (email && password) {
    ...
    return new Response(JSON.stringify({ redirect: "/admin/dashboard" }), { status: 200 });
  }
